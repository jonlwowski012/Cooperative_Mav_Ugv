...
std::string	modelString_black_cylinder;
std::string	modelString_black_box;
std::string	modelString_yellow_box;
...
void delete_obstacle_set_4(){
	dreq.model_name = "black_cylinder_1";
	deleter.call(dreq,dresp);
	dreq.model_name ="black_cylinder_2";
	deleter.call(dreq,dresp);
	dreq.model_name ="black_cylinder_3";
	deleter.call(dreq,dresp);
	dreq.model_name ="black_cylinder_4";
	deleter.call(dreq,dresp);
	dreq.model_name ="black_box_1";
	deleter.call(dreq,dresp);
	dreq.model_name ="yellow_box_1";
	deleter.call(dreq,dresp);
}
void spawn_obstacle_set_4(){
	req.model_name = "black_cylinder_1";
	req.reference_frame = "world";
	req.model_xml = modelString_black_cylinder;
	req.initial_pose.position.x = -4;
	req.initial_pose.position.y = -2;
	req.initial_pose.position.z = 1.5;
	spawner.call(req,resp);
	req.model_name = "black_cylinder_2";
	req.reference_frame = "world";
	req.model_xml = modelString_black_cylinder;
	...
	req.initial_pose.position.z = 1.5;
	spawner.call(req,resp);
	req.model_name = "black_cylinder_3";
	req.reference_frame = "world";
	req.model_xml = modelString_black_cylinder;
	req.initial_pose.position.x = -4;
	req.initial_pose.position.y = -8;
	req.initial_pose.position.z = 1.5;
	spawner.call(req,resp);
	req.model_name = "black_cylinder_4";
	req.reference_frame = "world";
	req.model_xml = modelString_black_cylinder;
	req.initial_pose.position.x = 2;
	req.initial_pose.position.y = -2;
	req.initial_pose.position.z = 1.5;
	spawner.call(req,resp);
	req.model_name = "black_box_1";
	req.reference_frame = "world";
	req.model_xml = modelString_black_box;
	req.initial_pose.position.x = -1;
	req.initial_pose.position.y = -5.4;
	req.initial_pose.position.z = 3;
	spawner.call(req,resp);
	req.model_name = "yellow_box_1";
	req.reference_frame = "world";
	req.model_xml = modelString_yellow_box;
	req.initial_pose.position.x = 2.8;
	req.initial_pose.position.y = -10.2;
	req.initial_pose.position.z = 1.5;
	req.initial_pose.orientation.x = 0;
	req.initial_pose.orientation.y = 0;
	req.initial_pose.orientation.z = 1.5;
	req.initial_pose.orientation.w = 1.5;
	spawner.call(req,resp);
	req.model_name = "yellow_box_2";
	req.reference_frame = "world";
	req.model_xml = modelString_yellow_box;
	req.initial_pose.position.x = -1.15;
	req.initial_pose.position.y = -10.2;
	req.initial_pose.position.z = 1.5;
	req.initial_pose.orientation.x = 0;
	req.initial_pose.orientation.y = 0;
...
double goto_target(double x, double y){
	...
	while (diff_x > .1 || diff_y>.1){
		...
		goal_x = x - ugv_pose_x;
		goal_y = y - ugv_pose_y;
		if(mean_blue[0] > 0.1 || mean_green[0] > 0.1 || mean_pink[0] > 0.1 || mean_yellow[0] > 0.1){
			return 0;
		}
		ros::spinOnce();
		loop_rate.sleep();
...
double rotateto_target(double x, double y){
	ros::Rate loop_rate(50);
	loop_rate.sleep();
	ros::spinOnce();
	double D_yaw = 1;   
	...
	double Kd_yaw = 1;
	double angle;
	double diff_yaw=1000;
	double sign_yaw;
	double P_yaw;
	double PD_yaw;
	
	while (diff_yaw>.02){
		angle = rotate_target(x,y);
		// ******** YAW PID  ********* //
		diff_yaw = angle_difference(ugv_yaw,angle);
		sign_yaw = turn_direction(ugv_yaw, angle);
		P_yaw = sign_yaw*diff_yaw*.05;
		...
		Derivator_yaw= error_yaw;
		Kd_yaw = D_yaw;
		cout << "diff yaw= " << diff_yaw << endl;
		set_velocity_ugv(0,PD_yaw);
		ros::spinOnce();
		loop_rate.sleep();
	}
...
void canopy_controller( double x , double y){
	reset_communication();
		...
		if(uav_in_room == 1){
			if(mean_yellow[0] > 10){
				if(objects_detected == 1){
					P_yaw=((.007)*nxc_yellow);
					error_yaw = nxc_yellow;
					D_yaw = Kd_yaw * (error_yaw - Derivator_yaw);
					PD_yaw = P_yaw+D_yaw;
					Derivator_yaw = error_yaw;
					Kd_yaw = D_yaw;
					cout<< "nxc inv = " << nxc_yellow << "PD= " << PD_yaw << endl;
...
void maincontroller(double x, double y){
	...
	while (finished != 1){
		...
		loop_rate.sleep();
		ros::spinOnce();
		cout << "Mean Yellow= " << mean_yellow[0] << endl;
		if(mean_yellow[0] > 0.1){
			canopy_controller(x,y);
		}
		else if(mean_blue[0] > 0.1 && obstacle1_passed == 1){
...
void combined_controller(){
/********* Reading Models for Objects   *************************/
		modelString_blue = loadModelSDF("package://cooperative_obstacle_avoidance/urdf/blue_cylinder.urdf");
		modelString_green = loadModelSDF("package://cooperative_obstacle_avoidance/urdf/green_cylinder.urdf");
		modelString_pink = loadModelSDF("package://cooperative_obstacle_avoidance/urdf/purple_cylinder.urdf");
		modelString_orange = loadModelSDF("package://cooperative_obstacle_avoidance/urdf/orange_box.urdf");
		modelString_black_cylinder = loadModelSDF("package://cooperative_obstacle_avoidance/urdf/black_cylinder.urdf");
		modelString_black_box = loadModelSDF("package://cooperative_obstacle_avoidance/urdf/black_box.urdf");
		modelString_yellow_box = loadModelSDF("package://cooperative_obstacle_avoidance/urdf/yellow_box.urdf");
		/********* Deleting Obstacles   *************************/
		delete_obstacles_all();
