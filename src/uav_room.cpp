...
void track_ugv(double time){ 
    ...
    for(q=0; q<time; q++){
		...
		if((door_seen == 0 && door_seen == 1) || (door_seen == 1 && uav_in_room ==1)){
	  ...
	  // ############# Roll PID      ##################### 
		...
		else if ((door_seen == 1 || door_seen == 0) && uav_in_room==0){
			while(nyc_yellow >= 0 || mean_yellow[0] < 3){
				...
				double error_roll = 0;
				double Kd_roll = 1;
				double Derivator_Alt = 0;
				double error_Alt = 0;
				double Kd_Alt = 1;
				double direction_door;
				double rotate_uav;
				cout<<"Mean yellow: " << mean_yellow[0] << "Mean Turquoise:  "<< mean_turquoise[0] << endl;
				while(mean_yellow[0] > 1 || mean_turquoise[0] > 1){
					if(mean_yellow[0] > 1 && mean_turquoise[0] > 1){
						// ############# Roll PID      ##################### 
						double P_roll=((.01)*((nxc_yellow+nxc_turquoise)/2));
						error_roll = ((nxc_yellow+nxc_turquoise)/2);
						double D_roll = Kd_roll * (error_roll - Derivator_roll);
						double PD_roll = P_roll+(1.02*D_roll);
						Derivator_roll = error_roll;
						Kd_roll = D_roll;
						
						// ############# Altitude PID      ##################### 
						double P_Alt=((.01)*(nyc_yellow));
						error_Alt = nxc_yellow;
						double D_Alt = Kd_Alt * (error_Alt - Derivator_Alt);
						double PD_Alt = P_Alt+(1.1*D_Alt);
						Derivator_Alt = error_Alt;
						Kd_Alt = D_Alt;
						
						set_velocity_uav(.5,-PD_roll,-PD_Alt,0);
					}
					else if (mean_yellow[0] > 1 && mean_turquoise[0] < 1){
						set_velocity_uav(.2,-.2,0,0);
					}
					else if (mean_yellow[0] < 1 && mean_turquoise[0] > 1){
						set_velocity_uav(.2,.2,0,0);
					}
